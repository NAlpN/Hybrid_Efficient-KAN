...
def seed_everything(seed=42):
    """
    Rastgeleliği ortadan kaldırır. Sonuçları tekrarlanabilir yapar.
    """
    random.seed(seed)
    ...
    print(f"[SİSTEM] Random Seed {seed} olarak kilitlendi. Sonuçlar sabit.")
...
CONFIG = {
    ...
    "save_dir": f"experiments/final_no_tta_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
}
...
class RobustEyeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if patient_path is None:
            return torch.zeros((7, 3, 224, 224)), label, "ALL_MISSING"
            ...
            if not force_black and files:
                for img_path in files:
                    try:
                        ...
                        current_mod_tensors.append(torch.zeros((3, 224, 224)))
...
def run_pipeline():
    ...
    # --- Eğitim Döngüsü ---
    print(f"--- {CONFIG['k_folds']}-Fold Transfer Learning & Zero Padding Eğitim Başlıyor ---")
    ...
    with torch.no_grad():
        for imgs, lbls, missing_info in tqdm(test_loader_final, desc="Analiz Ediliyor"):
            ...
            for i in range(len(lbls)):
                is_correct = (pred[i].item() == lbls[i].item())
                detailed_results.append({
                    "True_Label": ['Hemangioma', 'Melanoma', 'Metastatic'][lbls[i].item()],
                    "Pred_Label": ['Hemangioma', 'Melanoma', 'Metastatic'][pred[i].item()],
                    "Confidence": conf[i].item(),
                    "Correct": is_correct,
                    "Missing_Modalities": missing_info[i],
                    "TTA_Applied": False # TTA olmadığı için hep False
                })
                
                if is_correct: correct_confs.append(conf[i].item())
                else: wrong_confs.append(conf[i].item())
    ...
    plt.title('Final Confusion Matrix (Zero Padding - No TTA)')
    ...
    print(f"\nFinal Test Accuracy (No TTA): %{acc*100:.2f}")
