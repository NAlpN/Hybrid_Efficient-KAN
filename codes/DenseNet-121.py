...
CONFIG = {
    ...
    "save_dir": "experiments/densenet_frozen_no_tta"
}
...
# ==========================================\
# MODEL: Frozen DenseNet-121 + KAN
# ==========================================\
class HybridDenseNetKAN(nn.Module):
    def __init__(self, num_classes=3):
        super(HybridDenseNetKAN, self).__init__()
        ...
        self.projection = nn.Linear(3072, 64)
        self.relu = nn.ReLU()
    ...
    def forward(self, img_fa, img_icga, img_us):
        feat_fa = self.flatten(self.pool(F.relu(self.backbone(img_fa))))     # [Batch, 1024]
        feat_icga = self.flatten(self.pool(F.relu(self.backbone(img_icga)))) # [Batch, 1024]
        feat_us = self.flatten(self.pool(F.relu(self.backbone(img_us))))     # [Batch, 1024]
        
        combined_features = torch.cat((feat_fa, feat_icga, feat_us), dim=1) # [Batch, 3072]
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(df)), df['pathology'])):
    print(f"\n--- FOLD {fold+1}/{CONFIG['k_folds']} (Frozen DenseNet-121 + KAN) ---")
    ...
    model = HybridDenseNetKAN(num_classes=3).to(CONFIG['device'])
...
plt.title('DenseNet-121 + KAN')
plt.savefig(os.path.join(CONFIG['save_dir'], 'matrix_densenet_frozen.png'))
pd.DataFrame({'True_Label': all_true_labels, 'Pred_Label': all_pred_labels}).to_csv(os.path.join(CONFIG['save_dir'], 'preds_densenet_frozen.csv'), index=False)