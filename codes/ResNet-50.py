...
CONFIG = {
    ...
    "save_dir": "experiments/resnet50_frozen_no_tta"
}
...
class MultimodalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for mod in self.modalities:
            ...
            img_tensor = torch.zeros((3, CONFIG['image_size'], CONFIG['image_size'])) # ZERO PADDING
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(df)), df['pathology'])):
    print(f"\n--- FOLD {fold+1}/{CONFIG['k_folds']} (Frozen ResNet-50 + KAN) ---")
...
plt.title('ResNet-50 + KAN')
plt.savefig(os.path.join(CONFIG['save_dir'], 'matrix_resnet_frozen.png'))
pd.DataFrame({'True_Label': all_true_labels, 'Pred_Label': all_pred_labels}).to_csv(os.path.join(CONFIG['save_dir'], 'preds_resnet_frozen.csv'), index=False)